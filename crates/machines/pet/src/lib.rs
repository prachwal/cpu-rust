...
impl PetBus {
    fn io_read(&mut self, addr: u16) -> u8 {
        match addr {
            ...
            0xE840..=0xE84F => self.via.read(addr),
            _ => 0,
        }
    }
...
impl Pet2001 {
    ...
    pub fn run(&mut self, count: u32) -> u32 {
        ...
        for _ in 0..count {
            ...
            // VIA tick — advance timers by actual cycle count
            self.bus.via.tick(cycles);

            // Vertical blank: CB1 edge ~60Hz (1 MHz / 60 ≈ 16666 cycles)
            self.bus.vb_counter += cycles as u32;
            if self.bus.vb_counter >= 16666 {
                self.bus.vb_counter -= 16666;
            ...
            // IRQ delivery
            if self.bus.via.irq {
                if self.cpu.trigger_irq_bus(&mut self.bus) {
                    self.bus.via.ifr_clear(via_6522::IRQ_CB1);
                }
            }
            n += 1;
        }
    ...
    pub fn chargen_ptr(&self) -> *const u8 {
        self.chargen.as_ptr()
    }
